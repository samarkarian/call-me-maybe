...
def ft_prompt(prompt: dict[str, str], definition: list[Any]) -> str:
    """Build a prompt string describing available functions and the user request.
    ...
    for d in definition:
        ...
        lines.append(
            f"- {d['name']}({params}) -> {d['returns']['type']}: {d['description']}"
        )
...
def main_prompt(definition: list[Any], prompt: list[Any]) -> None:
    ...
    """
    promp = random.choice(prompt)
    encode = ft_prompt(promp, definition)
